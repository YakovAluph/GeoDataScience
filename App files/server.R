...
shinyServer(function(input, output, session) {
  ...
  plotBtnPress <- reactiveVal(FALSE)
  krigingReady <- reactiveVal(TRUE)
  
  # --- UI Reset Functions / File Input Observers --- #
  
  resetAppState <- function(session) {
  ...
  output$mainTabs <- renderUI({
    if (clearedUI()) {
      ...
      tabsetPanel(
        ...
        tabPanel("Kriging Prediction",
                 conditionalPanel("!input.quickPreview",
                                  h4("Kriging"),
                                  uiOutput("krigingPlotMessage"),
                                  plotOutput("krigingPlot"),
                                  uiOutput("krigingSaveNote"),
                                  hr(),
                                  h4("Map"),
                                  uiOutput("krigingMapMessage"),
  ...
  output$voronoiPlot <- renderPlot({
    ...
    df <- cleanAndPrepareData(); req(df)
    make_voronoi_plot(df, input$xcol, input$ycol, input$valcol)
  })
  ...
  output$krigingSaveNote <- renderUI({
    if (krigingReady()) {
      div(style = "color: gray; font-size: 13px; margin-top: 5px;",
          "💾 Right click on the image to copy or save image on your device.")
    }
  ...
  observeEvent(input$plotBtn, {
    ...
    uiOutput("voronoiPlot")
    
    # --- Step 4: Threshold Check (for IK) --- #
    # this will be in kriging.R in future push
