...
shinyServer(function(input, output, session) {
  ...
  output$columnSelectors <- renderUI({
    ...
    cols <- names(userData())
    tagList(
      selectizeInput("xcol",  "X Coordinate", choices = cols),
      selectizeInput("ycol",  "Y Coordinate", choices = cols),
      selectizeInput("valcol","Variable",     choices = cols)
    )
  })
  ...
  observeEvent(input$plotBtn, {
    ...
    withProgress(message = "Running Analysis...", value = 0.1, {
      ...
      customPalette <- colorNumeric(
        palette = c("#d73027", "#fc8d59", "#fee090", "#ffffbf",
                    "#e0f3f8", "#91bfdb", "#4575b4", "#6e40aa"),
      ...
      output$krigingPlot <- renderPlot({
        ggplot(ddf, aes(x = x, y = y, fill = var1.pred)) +
          ...
          scale_fill_gradientn(
            colors = rev(c("#6e40aa", "#4575b4", "#91bfdb", "#e0f3f8",
